...
class ProcessGroupManager:
    def __init__(self, dp_size, pp_size, tp_size):
        ...
        self.pp_next_rank = None if self.pp_is_last_stage else int(self.grid[self.dp_rank, self.pp_rank + 1, self.tp_rank].item())
        self.pp_prev_rank = None if self.pp_is_first_stage else int(self.grid[self.dp_rank, self.pp_rank - 1, self.tp_rank].item())
        # DP
...
def setup_process_group_manager(dp_size, pp_size, tp_size):
    ...
    process_group_manager = ProcessGroupManager(dp_size=dp_size, pp_size=pp_size, tp_size=tp_size)
